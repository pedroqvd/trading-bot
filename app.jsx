// =====================================================================
// Main App + sidebar nav + page routing + Live Feed + footer + density
// =====================================================================
...
function Sidebar({ page, setPage, status }) {
  const items = [
    { id: "dashboard", label: "Painel", ico: "▣", kbd: "1" },
    { id: "analysis", label: "Análise", ico: "◉", kbd: "2" },
    { id: "settings", label: "Configurações", ico: "⚙", kbd: "3" },
    { id: "backtest", label: "Backtest", ico: "▶", kbd: "4" },
    { id: "logs", label: "Logs", ico: "≡", kbd: "5" },
  ];
    ...
    <aside className="sidebar">
      ...
      {items.map((it) => (
        ...
        >
          ...
          <span className="kbd" style={{ marginLeft: "auto" }}>{it.kbd}</span>
        </button>
      ))}
...
function DashboardPage({
  period, setPeriod, status, statusRefetch,
  onStart, onStopRequest, onShowError, setOpenPos,
  onCmd, refetchAll, pushToast, onJumpToLogs,
}) {
  const metrics = usePoll(() => MockAPI.getMetrics(period), 5_000, [period]);
  const equity = usePoll(() => MockAPI.getEquity(period), 5_000, [period]);
  const equityEvents = usePoll(() => MockAPI.getEquityEvents(), 30_000, []);
  const stratSparklines = usePoll(() => MockAPI.getStratSparklines(), 30_000, []);
  ...
  React.useEffect(() => {
    refetchAll.current = () => {
      ...
      positions.refetch(); trades.refetch();
      risk.refetch(); markets.refetch();
    };
  });
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginBottom: 12 }}>
        <StrategyTable
          rows={metrics.data?.by_strategy}
          sparklines={stratSparklines.data}
          refetching={metrics.refetching}
        />
      </div>
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="card-header">
          <div className="card-title">Mapa de calor de mercados</div>
          <span className="card-sub">tamanho ∝ √volume · destaque verde = candidato a arbitragem</span>
        </div>
        <MarketsHeatmap markets={markets.data?.markets?.slice(0, 24)} onPickMarket={() => {}} />
      </div>
...
function AppInner() {
  ...
  const [logsCidFilter, setLogsCidFilter] = React.useState(null);
  ...
  const onJumpToLogs = (cid) => {
    setLogsCidFilter(cid);
    setPage("logs");
    pushToast(`Logs filtrados por ${cid}`, "warn");
  };
  const { dense } = React.useContext(DensityCtx);
  ...
  React.useEffect(() => {
    const onKey = (e) => {
      ...
      // Page nav: 1-5 (no modifier)
      if (!meta && !e.altKey && !e.shiftKey && document.activeElement?.tagName !== "INPUT") {
        if (e.key === "1") setPage("dashboard");
        if (e.key === "2") setPage("analysis");
        if (e.key === "3") setPage("settings");
        if (e.key === "4") setPage("backtest");
        if (e.key === "5") setPage("logs");
        if (e.key.toLowerCase() === "s") {
          if (status.data?.running) setStopOpen(true);
          else onStart();
        }
        if (e.key.toLowerCase() === "d") {
          // toggle density via a synthetic class flip
          document.documentElement.classList.toggle("dense");
          localStorage.setItem("dense", document.documentElement.classList.contains("dense") ? "1" : "0");
        }
      }
      if (e.key === "Escape") {
  ...
  }, [cmdOpen, stopOpen, errOpen, status.data?.running]);
  ...
  const commands = [
    ...
    { label: "Ir para o painel", run: () => setPage("dashboard"), kbd: "1" },
    { label: "Ir para análise", run: () => setPage("analysis"), kbd: "2" },
    { label: "Ir para configurações", run: () => setPage("settings"), kbd: "3" },
    { label: "Ir para backtest", run: () => setPage("backtest"), kbd: "4" },
    { label: "Ir para logs", run: () => setPage("logs"), kbd: "5" },
    ...
    { label: "Alternar densidade da tabela", run: () => { document.documentElement.classList.toggle("dense"); }, kbd: "D" },
  ];

  return (
    <div className={`app-shell with-feed ${dense ? "dense" : ""}`}>
      ...
      <main>
        ...
        {page === "logs" && (
          <LogsPage
            cidFilter={logsCidFilter}
            onClearCid={() => setLogsCidFilter(null)}
          />
        )}
      </main>
      ...
      {openPos && (
        <PositionDetailSheet
          ...
          onCloseNow={(id) => {
            const updated = MockAPI.closePosition(id);
            if (updated) {
              pushToast(`Posição #${id} encerrada — PnL ${Fmt.usdPrecise(updated.realized_pnl_usd)}`, "warn");
              setOpenPos(null);
              refetchAll.current();
              status.refetch();
            }
